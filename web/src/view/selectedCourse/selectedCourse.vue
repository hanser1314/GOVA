<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column sortable align="left" label="学号" prop="sno" width="120" />
...
import {
  ...
  deleteSelectedCourseByIds,
  updateSelectedCourse,
  findSelectedCourse,
  getSelectedCourseList
} from '@/api/selectedCourse'
...
import { getSctByUsername } from '@/api/sct'
...
const getTableData = async() => {
  // const table = await getSelectedCourseList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  // if (table.code === 0) {
  ...
  // }
  const table = await getSctByUsername({ page: page.value, pageSize: pageSize.value, sno: username.value })
  if (table.code === 0) {
    tableData.value = table.data.list
    total.value = table.data.total
    page.value = table.data.page
...
const onDelete = async() => {
      ...
      const res = await deleteSelectedCourseByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateSelectedCourseFunc = async(row) => {
    const res = await findSelectedCourse({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.reselectedCourse
...
// 删除行
const deleteSelectedCourseFunc = async (row) => {
    const res = await deleteSelectedCourse({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createSelectedCourse(formData.value)
                  break
                case 'update':
                  res = await updateSelectedCourse(formData.value)
                  break
                default:
                  res = await createSelectedCourse(formData.value)
                  break
